...
template <class Algorithm> struct StringRunner {
public:
  template <class... Args>
...
static v8::Local<v8::Object> ProcessKeys(v8::Local<v8::Object> object,
                                         Args... args) {
  Nan::EscapableHandleScope scope;
  auto newObject =
      InPlace ? object
              : (object->IsArray() ? Nan::New<v8::Array>().As<v8::Object>()
                                   : Nan::New<v8::Object>());
  ...
  if (object->IsArray()) {
    ...
    for (uint i = 0; i < objectArray->Length(); ++i) {
      ...
      if (value->IsObject() && !value->IsDate()) {
        newValue = ProcessKeys<KeyProcessor>(value.As<v8::Object>(), args...)
...
template <class Algorithm> static NAN_METHOD(XCaseKeys) {
  if (!info[0]->IsObject()) {
  ...
  auto retValue = inPlace ? ProcessKeys<StringRunner<Algorithm>, true>(object)
                          : ProcessKeys<StringRunner<Algorithm>, false>(object);

  info.GetReturnValue().Set(retValue);
}

template <class Algorithm> static NAN_METHOD(DeXCaseKeys) {
  if (!info[0]->IsObject()) {
    info.GetReturnValue().Set(info[0]);
    return;
  }
  ...
  if (info[1]->IsObject()) {
    ...
    if (options->Has(separatorKey)) {
      ...
      separatorString->Write((uint16_t *)&separator, 0, 1,
  ...
  auto retValue =
      inPlace ? ProcessKeys<StringRunner<Algorithm>, true>(object, separator)
              : ProcessKeys<StringRunner<Algorithm>, false>(object, separator);

  info.GetReturnValue().Set(retValue);
}

template <class Algorithm> static NAN_METHOD(XCaseMethod) {
  if (!info[0]->IsString()) {
...
template <class Algorithm> static NAN_METHOD(DeXCaseMethod) {
  char16_t separator = L'_';
  ...
  if (info[1]->IsObject()) {
    ...
    if (options->Has(separatorKey)) {
      auto separatorString = info[1]
                                 ->ToObject()
                                 ->Get(Nan::New("separator").ToLocalChecked())
                                 ->ToString();
      if (separatorString->Length() > 1) {
        Nan::ThrowTypeError("Separator should be a single character");
        return;
